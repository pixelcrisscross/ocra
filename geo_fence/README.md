...
## Geofence data bootstrap

The repository includes `scripts/sync_geofences.py`. Run it from the project root to create/update `data/geofences.geojson`:

```powershell
python scripts/sync_geofences.py
```

It retrieves reference EEZ polygons from a Marine Regions-backed ArcGIS feature service and coastal protected-area candidate geometries from OpenStreetMap. The file preserves source URLs and marks these as reference/verification layers; it must not be treated as an official nautical chart or legal boundary source. India Code contains the national maritime-zone legislation and notifications, while WII/ENVIS publishes India's protected-area information.  

The `/api/geofence/check` endpoint now only returns zones containing the point or within the requested proximity (default 25 km), and returns the source URL and legal-authority status for each hit.
